  ...
  return (
    <div className="flex mb-[22px] rounded-[7px] overflow-hidden shadow-[0_2px_12px_rgba(26,26,46,.07)]">
      {STEPS.map((label, i) => {
        ...
        const isDone = i < cur
        const volledig = tabVolledig(state, i)
        return (
          <button
            key={i}
            type="button"
            onClick={() => goTo(i)}
            title={volledig ? 'Tabblad compleet' : 'Nog niet compleet'}
          ...
          >
            ...
            {label}
            <span className={`block mx-auto mt-1 w-1.5 h-1.5 rounded-full ${volledig ? 'bg-white' : 'bg-warn'}`}></span>
          </button>
        )
      })}
    </div>
