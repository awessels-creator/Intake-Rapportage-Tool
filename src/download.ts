...
export async function downloadWord(state: FormState) {
  ...
  if (state.inkomenData.length > 0) {
    ...
    children.push(simpleTable(['Bron / Werkgever', 'Type', 'Netto/mnd', 'Dienstverband', 'Beslag?'], state.inkomenData.map(d => [d.bron || '—', d.type || '—', `€ ${nl(parseFloat(d.netto) || 0)}`, d.uren || '—', d.beslag ? 'Ja' : 'Nee'])))
  }
  const beslagData = state.beslagData.filter(b => b.wie)
  if (beslagData.length > 0) {
    children.push(h3('Beslagleggende schuldeisers'))
    children.push(simpleTable(['Schuldeiser', 'Soort', 'Bedrag'], beslagData.map(b => [b.wie, b.soort || '—', b.bedrag ? `€ ${b.bedrag}/mnd` : 'Onbekend'])))
  }
  ...
  children.push(simpleTable(
    ...
    TOESLAGEN.map(t => {
      const actief = state.toeslagenActief[t]; const v = state.toeslagenBedrag[t]; const bes = state.toeslagenBeslag[t]
  ...
  const lastenRows = allDef
    .filter(row => { if (row.autoOnly && state.heeft_auto !== 'ja') return false; if (row.kinderOnly && !hK) return false; const w = state.lastenWaarden[row.id]; return w && parseFloat(w.bedrag) > 0 })
    .map(row => { const w = state.lastenWaarden[row.id]; const bdr = parseFloat(w.bedrag) || 0; const factor = PER_OPTIES.find(p => p.v === w.per)?.f || 1; const mnd = bdr * factor; return [row.post, `€ ${nl(bdr)} ${PER_OPTIES.find(p => p.v === w.per)?.l || '/mnd'}`, `€ ${mnd.toFixed(2)}/mnd`, w.opm || ''] })
  ...
  if (schuldenData.length > 0) {
    children.push(simpleTable(['Schuldeiser', 'Soort', 'Openstaand', 'Aflossing', 'Preferent', 'Schone lei?', 'Status'], schuldenData.map(s => [s.s || '—', (s.t || '—') + (s.subt ? ` (${s.subt})` : ''), `€ ${nl(parseFloat(s.b) || 0)}`, s.afl ? `€ ${s.afl}/mnd` : '—', (SCHULD_INFO[s.t] || {}).pref || '—', (SCHULD_INFO[s.t] || {}).lei || '—', s.st || '—'])))
  ...
  const diensten: [string, string][] = (['budgetbeheer', 'schuldregeling', 'bewind_medisch', 'bewind_schuld', 'schuldhulpmaatje'] as const)
    .filter(k => state[`cb_${k}` as keyof FormState])
    .map(k => [{ budgetbeheer: 'Budgetbeheer', schuldregeling: 'Schuldregeling', bewind_medisch: 'Beschermingsbewind (medisch)', bewind_schuld: 'Schuldenbewind', schuldhulpmaatje: 'Schuldhulpmaatje/Humanitas' }[k] || k, 'Ja'])
  ...
  // Use toBuffer which works in browser, then convert to Blob
