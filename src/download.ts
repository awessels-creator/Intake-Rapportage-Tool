...
export function downloadWord(state: FormState) {
  ...
  const lastenRows = allDef
    ...
    .map(row => {
      ...
      return `<tr><td>${row.post}</td><td>€ ${nl(bdr)} ${PER_OPTIES.find(p => p.v === w.per)?.l || '/mnd'}</td><td>€ ${mnd.toFixed(2)}/mnd</td><td>${w.opm || ''}</td></tr>`
    })
    .join('')
  ...
  const verzRows = verzItems.map(item => `<tr><td>${item.n}</td><td style="font-weight:600">${verzTekst(item.v)}</td></tr>`).join('')
...
<head>
<meta charset="UTF-8">
<title>Intakerapportage ${naam}</title>
<style>
body{font-family:Arial,sans-serif;font-size:11pt;color:#111111;line-height:1.4;margin:0}
h1{font-size:16pt;color:#111111;border-bottom:3px solid #ea5403;padding-bottom:4px;margin:12px 0 6px 0}
h2{font-size:12pt;color:#ea5403;border-bottom:1px solid #e0e0e0;padding-bottom:2px;margin:10px 0 4px 0}
h3{font-size:10pt;color:#555555;margin:6px 0 2px 0}
p{margin:2px 0;font-size:10pt}
table{border-collapse:collapse;width:100%;font-size:9.5pt;margin:4px 0}
th{background:#ea5403;color:#fff;padding:3px 6px;text-align:left;font-weight:bold}
td{padding:3px 6px;border:1px solid #e0e0e0}
tr:nth-child(even) td{background:#f2f2f2}
.meta{color:#666;font-size:9pt}
.footer{margin-top:12px;border-top:1px solid #e0e0e0;padding-top:4px;font-size:8.5pt;color:#888}
</style>
</head>
<body>
...
${state.heeft_partner === 'ja' ? `<h3>Partner</h3><table><tr><td style="width:150px">Naam</td><td>${state.partner_vnaam || ''} ${state.partner_anaam || ''}</td><td style="width:120px">Geboortedatum</td><td>${state.partner_geb || '—'}</td></tr><tr><td>BSN partner</td><td>${state.partner_bsn || '—'}</td><td>In regeling?</td><td>${state.partner_reg || '—'}</td></tr></table>` : ''}
...
${state.persoonlijk ? `<h3>Woonsituatie / netwerk</h3><p>${state.persoonlijk}</p>` : ''}
${state.opleiding_toel ? `<h3>Opleiding / werkervaring / perspectief</h3><p>${state.opleiding_toel}</p>` : ''}
${state.flank === 'ja' ? `<h3>Flankerende hulpverlening</h3><p>${state.flank_inst || '—'} — ${state.flank_naam || '—'} (${state.flank_contact || '—'}) — ${state.flank_aard || '—'}</p>` : ''}
...
<tr><td>Eerdere aanvragen</td><td>${state.eerder_aanvr === 'ja' ? 'Ja — ' + (state.eerder_aanvr_toel || '') : 'Nee'}</td></tr>
</table>
<h3>Reden aanmelding / hulpvraag</h3><p>${state.hulpvraag || '—'}</p>

<h2>4. Bankrekening(en)</h2>
${bankRows ? `<table><thead><tr><th>IBAN</th><th>Tenaamstelling</th><th>Type</th><th>Saldo</th><th>Roodstand</th><th>Nieuwe rek. besproken</th></tr></thead><tbody>${bankRows}</tbody></table>` : '<p>Geen bankrekeningen geregistreerd.</p>'}
${state.bank_toelichting ? `<p><em>${state.bank_toelichting}</em></p>` : ''}

<h2>5. Onderneming</h2>
<p>${!state.ondernemer || state.ondernemer === 'nee' ? 'Geen onderneming.' : state.ondernemer === 'actief' ? `Actief — ${state.kvk_naam || '?'}, KvK: ${state.kvk_nr || '?'}` : `Gestopt — ${state.kvk_naam || '?'}, KvK: ${state.kvk_nr || '?'}, uitgeschreven: ${state.kvk_datum || '?'}`}</p>
${state.ondernemer && state.ondernemer !== 'nee' ? `<p>Boekhouding: ${state.boekhouding || '—'} | Aangiften: ${state.aangifte || '—'}</p>${state.kvk_toel ? `<p><em>${state.kvk_toel}</em></p>` : ''}` : ''}

<h2>6. Vermogen</h2>
<table><tr><td style="width:150px">Spaargeld</td><td>€ ${nl(parseFloat(state.spaargeld) || 0)}</td><td style="width:120px">Beleggingen</td><td>€ ${nl(parseFloat(state.beleggingen) || 0)}</td></tr>
...
${state.vermogen_toel ? `<h3>Toelichting vermogen en verzekeringen</h3><p>${state.vermogen_toel}</p>` : ''}

<h2>7. Inkomen (per 1 jan 2026)</h2>
<table><tr><td style="width:150px">Bijstandsnorm</td><td>€ ${nl(norm)}/mnd</td><td style="width:120px">Totaal inkomen</td><td>€ ${nl(ink)}/mnd (${pct.toFixed(0)}% van norm)</td></tr>
...
<p>FDMA: ${state.fdma || '—'} | Kindsupport Meppel: ${state.kindsupport || '—'}</p>
...
<p>Kwijtschelding GBLT: ${state.kwgt || '—'} | Kwijtschelding gemeente: ${state.kwgm || '—'}</p>

<h2>9a. Beslagvrije Voet (indicatief, jan 2026)</h2>
<table><tr><td style="width:200px">Toe te passen BVV</td><td style="font-weight:bold">€ ${bvv.toLocaleString('nl-NL', { minimumFractionDigits: 2 })}</td><td style="width:200px">Max. voor beslag beschikbaar</td><td style="font-weight:bold;color:${ink - bvv > 0 ? '#1a4b2d' : '#9d3d1d'}">€ ${(ink - bvv).toLocaleString('nl-NL', { minimumFractionDigits: 2 })}</td></tr></table>

<h2>10. Schulden</h2>
${schuldenRows ? `<table><thead><tr><th>Schuldeiser</th><th>Soort</th><th>Openstaand</th><th>Aflossing</th><th>Preferent</th><th>Schone lei?</th><th>Status</th></tr></thead><tbody>${schuldenRows}</tbody></table><p style="font-weight:bold">Geschatte schuldenlast: € ${nl(schulden)}</p>` : '<p>Geen schulden geregistreerd.</p>'}
<p>Gezamenlijke schulden ex-partner: ${state.sch_exparter || '—'} | Voedselbank: ${state.voedselbank || '—'}</p>
${state.schulden_opm ? `<p><em>${state.schulden_opm}</em></p>` : ''}
...
${['budgetbeheer', 'schuldregeling', 'bewind_medisch', 'bewind_schuld', 'schuldhulpmaatje', 'overig_aanvr'].some(k => state[`cb_${k}` as keyof FormState])
  ...
  : '<p>Geen dienstverlening geselecteerd.</p>'}

<h2>13. Adviezen & Actiepunten</h2>
${advHtml || '<p>Geen adviezen.</p>'}
${state.conclusie ? `<h2>14. Conclusie / Afspraken / Afsluiting</h2><p>${state.conclusie.replace(/\n/g, '<br>')}</p>` : ''}
...
<div class="footer">Rapportage: ${new Date().toLocaleDateString('nl-NL')} | Consulent: ${consulent} | Cliëntnr: ${state.clientnr || '—'} | Vertrouwelijk — Geldzorgen Schuldhulpverlening Meppel 2026</div>
  ...
  import('html-to-docx').then(({ default: htmlToDocx }) => {
    htmlToDocx(html, {
